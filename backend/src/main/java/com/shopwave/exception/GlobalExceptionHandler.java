...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleInventoryDown(InventoryServiceException ex) {
        log.warn("Inventory service error: {}", ex.getMessage());
        return problem(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage());
    }

    @ExceptionHandler(OrderPartialFailureDemoException.class)
    public ProblemDetail handlePartialFailureDemo(OrderPartialFailureDemoException ex) {
        log.error("LAB-6 partial failure demo triggered: {}", ex.getMessage());
        return problem(HttpStatus.CONFLICT, ex.getMessage());
    }
